...
- ✅ **小工具支持** - 应用列表、视频播放器、应用轮播小工具
- ✅ **短代码支持** - 灵活的短代码参数
- ✅ **视频播放器** - 支持手动添加视频或随机展示采集的视频
- ✅ **数据统计** - 完整的后台统计面板
- ✅ **系统诊断** - 健康检查和自动修复工具
- ✅ **使用教程** - 详细的使用说明文档
...
| `count` | 显示数量 | 数字 | 12 |
| `category` | 分类ID | 分类ID | - |
| `orderby` | 排序方式 | date, title, rand | date |
| `order` | 排序方向 | ASC, DESC | DESC |
| `show_view_more` | 显示查看全部 | true, false | true |
...
// 不显示查看全部按钮
[yyk_app_carousel show_view_more="false"]
```
...
// 完整参数
[yyk_video_player id="123" count="5"]
```
...
4. 配置参数：
   - 标题
   - 视频来源选择（随机展示采集的视频/手动添加视频）
   - 视频数量（1-20）
   - 自定义视频（每行一个，格式：视频地址|标题）
